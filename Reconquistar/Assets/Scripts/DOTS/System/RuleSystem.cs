    ...
    public partial struct RuleSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            if (behaviorTagQuery.IsEmpty)
            {
                ...
                // {
                //     SampleUnitComponentData temp = SystemAPI.GetComponent<SampleUnitComponentData>(sampleUnits[i]);
                //     Debug.Log(temp.team);
                // }
                ...
                //타일 배열
                //인덱스가 (3, 5)인 타일 = tiles[3 + 5 * mapMaker.number] => 세로로 바뀌었으니까 tiles[3 * number + 5]
                ...
                for (int i = 0; i < 2; i++)
                {
                        ...
                        .WithDisabled<AttackTag>().WithDisabled<MovingTag>().WithEntityAccess())
                    {
                        ...
                        //현재 유닛이 점거하고 있는 타일의 index정보를 받아옴 <기존 : tiles[unitIndex.x + unitIndex.y * mapMaker.number] => tiles[unitIndex.x * mapMaker.number + unitIndex.y]>
                        RefRW<MapTileAuthoringComponentData> currentTile = SystemAPI.GetComponentRW<MapTileAuthoringComponentData>(tiles[unitIndex.x * mapMaker.number + unitIndex.y]);
                        for (int j = 0; j < moves.Length; j++)
                        {
                            if (moves[j].x != 0 || moves[j].y != 0)
                            {
                                // 기존 : (unitIndex.x + moves[j].x) + (unitIndex.y + moves[j].y) * mapMaker.number 
                                RefRW<MapTileAuthoringComponentData> nextTile = SystemAPI.GetComponentRW<MapTileAuthoringComponentData>(tiles[(unitIndex.x + moves[j].x) * mapMaker.number + (unitIndex.y + moves[j].y)]);
                        ...
                        if (i == 0 && !SystemAPI.IsComponentEnabled<MovingTag>(entity)) // 첫 번째 루프(= 이동 여부를 판단하기 위한 루프)를 도는 중 MovingTag가 붙여지지 않은 entity의 경우 LazyTag 붙이기
